...
void instruct(uint8_t **text_segment, long index)
{
	...
	if (instruct_move(text_segment, op))
		return;
	else if (op_push(text_segment, op))
		return;
	else if (op_call(text_segment, op, (uint16_t) index))
		return;
	else if (op_add(text_segment, op))
		return;
	else if (op_jmp(text_segment, op, (uint16_t) index))
		return;
	else if (op_in(text_segment, op))
		return;
	else if (op_sbb(text_segment, op))
		return;
	else if (op_load(text_segment, op))
		return;
	else if (op_pop(text_segment, op))
		return;
	else if (op_or(text_segment, op))
		return;
	else if (op_cond_jmp(text_segment, op, (uint16_t) index))
		return;
	else if (op_cmp(text_segment, op))
		return;
	else if (op_byte(text_segment, op))
		return;
	else if (op_int(text_segment, op))
		return;
	else if (op_ret(text_segment, op))
		return;
	(*text_segment) += 2;
}
