...
int op_mov_3(uint8_t op, __attribute__((unused)) uint8_t flag,
	uint8_t byte2, uint8_t w)
{
	if (op == OP_W_MOV_3)
	{
...
int op_mov_4(uint8_t op, __attribute__((unused)) uint8_t flag,
	uint8_t byte2, uint8_t w)
{
	if (op == OP_W_MOV_4)
	{
...
int op_add_2(uint8_t op, __attribute__((unused)) uint8_t flag,
	uint8_t byte2, uint8_t w)
{
	if (op == OP_ADD_2)
	{
...
int op_adc_2(uint8_t op, __attribute__((unused)) uint8_t flag,
	uint8_t byte2, uint8_t w)
{
	if (op == OP_W_ADC_2)
	{
...
int op_xchg_0(
	uint8_t op, __attribute__((unused)) uint8_t flag, uint8_t byte2, uint8_t w)
{
	if (op == OP_XCHG_0)
	{
		PC += 2;
		...
		set_data(pdata.data_left, w, rdata);
		set_data(pdata.data_right, w, ldata);
...
int op_test_0(uint8_t op,  __attribute__((unused)) uint8_t flag,
	uint8_t byte2, uint8_t w)
{
	if (op == OP_W_TEST_0)
	{
	...
	uint8_t byte2, uint8_t w)
{
	if (op == OP_W_OR_1 && flag == OP_W_OR_1_FLAG)
	{
		...
		ldata = get_data(pdata.data_left, w);
		result = ldata | pdata.data_right._imm16;	
		set_data(pdata.data_left, w, result);
	...
	uint8_t byte2, uint8_t w)
{
	if (op == OP_W_XOR_1 && flag == OP_W_XOR_1_FLAG)
	{
		...
		data = print_mr_data(OP_DONE_MARK"xor", byte2, w);

		uint16_t ldata, result;

		ldata = get_data(data.data_left, w);
		result = ldata ^ data.data_right._imm16;
...
int op_cmp_2(uint8_t op, __attribute__((unused)) uint8_t flag,
	uint8_t byte2, uint8_t w)
{
	if (op == OP_W_CMP_2)
	{
		...
		if (w == 1)
		{
			rdata = byte2 | (g_text_segment[PC + 2] << 8);
			sprintf(instr, OP_DONE_MARK"cmp ax, %04hx", rdata);
		}
		else
		{
...
int op_push_0(uint8_t op, uint8_t flag,
	uint8_t byte2, __attribute__((unused)) uint8_t w)
{
